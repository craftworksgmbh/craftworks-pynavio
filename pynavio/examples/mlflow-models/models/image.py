...
def _write_jpgs(images: np.ndarray, path: str):
    ...
    for i, image in enumerate(images):
        imwrite(path / f'img{i + 1}.jpg', image.reshape(*IMAGE_SHAPE))


class ImageModel(mlflow.pyfunc.PythonModel, TabularExplainerTraits):
    ...
    def _draw_plotly_explanation(self, image: np.ndarray,
        ...
        return json.loads(
            fig.update_traces(
                opacity=.8).add_layout_image(background).update_xaxes(
                    axis_spec,
                    range=[0, IMAGE_SHAPE[1] - 1
                          ]).update_yaxes(axis_spec,
                                          range=[IMAGE_SHAPE[0] - 1, 0],
                                          autorange=None).update_coloraxes(
                                              showscale=False).to_json())
...
def setup(with_data: bool,
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        dependencies = [np, pd, tf, PIL, './utils', './models']
        if explanations not in [None, 'disabled', 'default']:
            import IPython
            import matplotlib
            import plotly
            import shap
            dependencies.extend([shap, IPython])
            dependencies.append({
                'image': matplotlib,
                'plotly': plotly
            }[explanations])

        imwrite(f'{tmp_dir}/example.jpg', x_test[0].reshape(*IMAGE_SHAPE))
        example = {
            'image': imread(f'{tmp_dir}/example.jpg'),
            'digit': str(model.predict([x_test[[0]]]).argmax())
        }

        example_request = make_example_request(example, 'digit')
        example_request['featureColumns'][0]['type'] = 'image'

        to_mlflow(ImageModel(explanations),
                  example_request,
